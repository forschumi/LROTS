...
########################################
## Hardcopy support and generation checks
if { ([ string match -nocase "HARDCOPY*" $::family ]) && ($::IP(speedgrade) != "SG_3") } {
	post_message -type error "ALTMEMPHY IP that will be used on Hardcopy devices must be generated using a speedgrade of 3."
	return
} else {
	set current_speedgrade [get_speedgrade_string]
	if {[regexp -nocase "SG_(.+)" $::IP(speedgrade) match generated_speedgrade] > 0} {
		if {[regexp -nocase (.)*X $current_speedgrade match number] > 0} {
			set current_speedgrade $number
		} elseif {$current_speedgrade == "4L"} {
			set current_speedgrade "4"
		}
		if {$generated_speedgrade != $current_speedgrade} {
			post_message -type critical_warning "ALTMEMPHY IP was generated using a speedgrade of $generated_speedgrade, but is being compiled for a speedgrade of $current_speedgrade.  Timing analysis may not be valid due to violated timing model assumptions."
		}
	}
}
...
if {$companion_revision != ""} {
	set device_technology_migration_list [get_global_assignment -name device_technology_migration_list]
	if {([string match -nocase "HC3*" $device_technology_migration_list] || [string match -nocase "HC4*" $device_technology_migration_list])} {
		if {$::IP(speedgrade) != "SG_3"} {
			post_message -type error "ALTMEMPHY IP that will be migrated to Hardcopy devices must be generated using a speedgrade of 3."
			return
		}
	}
}
...
for {set inst_index 0} {$inst_index != [llength $instance_names]} {incr inst_index} {
	...
	foreach opc $opcs {
		...
		## Read Analysis
		if {$use_flexible_timing} {
			perform_flexible_read_capture_non_dqs_timing_analysis $opcs $opcname pins t summary MP IP board
		} else {
